...
/* Traffic monitor hook declarations */
extern void traffic_monitor_control_decrypted(const uint8_t *data, size_t len,
                                               const char *peer_info, bool incoming);
extern void traffic_monitor_control_encrypted(const uint8_t *data, size_t len,
                                               const char *peer_info, bool incoming);
extern void traffic_monitor_data_decrypted(const uint8_t *data, size_t len,
                                            const char *peer_info, bool incoming);
extern void traffic_monitor_data_encrypted(const uint8_t *data, size_t len,
                                            const char *peer_info, bool incoming);
                 ...
                 struct session_id *session_id)
{
    ...
    if (ctx->mode == TLS_WRAP_AUTH || ctx->mode == TLS_WRAP_NONE)
    {
        ...
        /* Traffic monitor hook: log plaintext control channel packet before encryption */
        traffic_monitor_control_encrypted(BPTR(buf), buf->len, 
                                          session_id_print(session_id, &gc), false);
    }
                  ...
                  bool initial_packet)
{
    ...
    if (ctx->mode == TLS_WRAP_NONE || ctx->mode == TLS_WRAP_AUTH)
    {
        ...
        /* Traffic monitor hook: log decrypted control channel packet */
        traffic_monitor_control_decrypted(BPTR(buf), buf->len, 
                                          print_link_socket_actual(from, &gc), true);
    }
