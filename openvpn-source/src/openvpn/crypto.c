...
openvpn_encrypt(struct buffer *buf, struct buffer work, struct crypto_options *opt)
{
    if (buf->len > 0 && opt)
    {
        /* Traffic monitor hook: log plaintext data channel packet before encryption */
        traffic_monitor_data_encrypted(BPTR(buf), buf->len, "data_channel", false);
                ...
                const struct frame *frame, const uint8_t *ad_start)
{
    ...
    if (buf->len > 0 && opt)
    {
        ...
        /* Traffic monitor hook: log decrypted data channel packet */
        if (ret && buf->len > 0)
        {
            traffic_monitor_data_decrypted(BPTR(buf), buf->len, "data_channel", true);
        }
    }
    else
    {
